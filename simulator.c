...
#define RFSIMULATOR_PARAMS_DESC {					\
    ...
    {"xsubport",             "<port to connect to xsubsocket>\n",              simOpt,  .u16ptr=&(rfsimulator->xsubsocket),           .defuintval=XSUBPORT,                 TYPE_UINT16,    0 },\
    {"xpubport",             "<port to connect to xpubsocket>\n",              simOpt,  .u16ptr=&(rfsimulator->xpubsocket),           .defuintval=XPUBPORT,                 TYPE_UINT16,    0 },\
...
typedef struct buffer_s {
  void *pub_sock;
  void *sub_sock; // temporary duplication of variables for testing
  int fd_pub_sock;
  int fd_sub_sock;
  // int conn_sock;
...
typedef struct {
  void *context; // temporary duplication of variables for testing
  void *pub_sock;
  void *sub_sock;
  int fd_pub_sock;
  int fd_sub_sock;
...
static void allocCirBuf(rfsimulator_state_t *bridge) {
...
.sin_addr=
  ...
  AssertFatal(epoll_ctl(t->epollfd, EPOLL_CTL_ADD,  t->listen_sock, &ev) != -1, "");// add the event to the epoll instance (listen sock will be monitored to check when data is ready to be read)
  return 0;
}
...
static bool flushInput(rfsimulator_state_t *t, int timeout, int nsamps_for_initial) {
  ...
  for (int nbEv = 0; nbEv < nfds; ++nbEv) {
    ...
    if (events[nbEv].events & EPOLLIN && fd == t->listen_sock) {// first connection
      ...
      rfsimulator_write_internal(t, t->lastWroteTS > 1 ? t->lastWroteTS-1 : 0,//qst why -1
                                 samplesVoid, 1,
                                 t->tx_num_channels, 1, false);// qst is the nbant the same as tx_num...
    } else {
