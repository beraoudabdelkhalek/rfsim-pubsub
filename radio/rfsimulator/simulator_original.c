...
static bool flushInput(rfsimulator_state_t *t, int timeout, int nsamps_for_initial)
{
  ...
  // read the connection topic,

  for (int nbEv = 0; nbEv < nfds; ++nbEv) { // for each initialized connection socket.
    // no need to check UUID array another time, dernaha in rfsimulator_read
    int fd = events[nbEv].data.fd;
    // check if it's a server : gNB, to enter to the connection request treatment part + set needed topic as Uplink.
    // otherwise if it's a client, yjouz y9ra 3adi + set brk needed topic as Downlink
    //  if there are any connection requests ( fetching x-sub and looking for "Connection" topic)
    if (events[nbEv].events & EPOLLIN && fd == t->listen_sock) { // == if there are any connection requests, we treat them by
                                                                 // publishing ack, generating uuid in the array, alloc cirbuf, set
                                                                 // connected_device = true ( allocCirbuf)
      ...
      // both devices enter here
      //  entered here from the have to wait mechanism, here when there is at least one UUID in the array...we receive real IQ
      //  samples ( for both devices gNb and UE)
...
static int rfsimulator_read(openair0_device *device, openair0_timestamp *ptimestamp, void **samplesVoid, int nsamps, int nbAnt)
{
  ...
  for (first_sock = 0; first_sock < FD_SETSIZE; first_sock++)
    if (t->buf[first_sock].circularBuf != NULL)
      break;
  // check if there is at least one UUID in the array ( or buffer containing connected_device == True )
  if (first_sock == FD_SETSIZE) {
    ...
    if (!flushInput(t, t->wait_timeout, nsamps)) {
      // here inside the flushInput we check if there are any connection request, if there are, we treat them in flushInput, if
      // flushInput return false so we read self generated samples..
    ...
    // receive the broadcasted data ( IQ samples )
    bool have_to_wait;
