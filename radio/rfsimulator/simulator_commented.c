...
#define PORT 7043 // default TCP port for this simulator
...
typedef struct buffer_s {
  // each iq sample is in 4 bytes ( 2*16 ),
  int conn_sock;
  openair0_timestamp lastReceivedTS;
  bool headerMode;
  bool trashingPacket;
  samplesBlockHeader_t th;
  char *transferPtr; // @ of the last data received nd settled in the circular buffer (ch7al raw m3mr the circular buffer).
  // the data here is IQ samples ( 4 bytes )...so we are receiving some bytes ( returned by recv, sent in write ( samplesToByte
  // bytes ) ) but we don't know how much exactly samples/bytes we received.
  uint64_t remainToTransfer;
  char *circularBufEnd; // @ of the end of the circular buffer.
  sample_t *circularBuf; // @ of the start of the circular buffer : where we will be storing the received data from this connection
                         // socket.
...
typedef struct {
  ...
  openair0_timestamp nextRxTstamp; // TS of when the next batch of IQ samples is expected to be received by the receiver...
  // ptimestamp : current timestamp of the current event "read".
  // nextrxtstamp: timestamp of the next event "read" that rfsimulator need to perfom next time.

  openair0_timestamp
      lastWroteTS; // TS of the last sent IQ samples // the last time (or sample index) when data was successfully sent

  // time stamp of when the rfsimulator finished its writing
  // written to the output buffer or sent over the air
  // they both indicates when the TX/RX channels/antennas will be available for future use ?
  // how they are updated :
  // in the transmission, when we transmit an rf signal ( of size nsamps ) in the antenna ( TX channel ), the
  // channel will be available ( released ) at lastWroteTS + size of the rfsignal ( current Timestamp + nsamps ) in
  // the reception, when we receive an rf signal ( of size nsamps ) in the antenna ( RX channel ), the channel will
  // be available again ( released ) at nextRxTstamp + size of the rfsignal ( current Timestamp + nsamps )

  uint64_t typeStamp;
  char *ip;
  uint16_t port;
  int saveIQfile; // to store the actual rf signal ( IQ samples ) we are sending.
  buffer_t buf[FD_SETSIZE];
  int rx_num_channels; // nbr of channels to use in reception
  int tx_num_channels; // nbr of channels to use in transmission = nbr antennas of the device.
  double sample_rate;
  double tx_bw;
  int channelmod;
...
{ // actual send of data ( header or rf signal ) through the connection socket fd.

  LOG_W(HW, "fullwrite start \n");
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  sleep(1);
  if (t->saveIQfile != -1) {
  ...
  LOG_I(HW, "sending %d bytes \n", count);

  while (count) {
    l = write(fd, buf, count);

    // count : nbr of bytes in buf, to send through fd.
    if (l <= 0) {
      if (errno == EINTR)
        continue;
...
static int startServer(openair0_device *device)
{
  ...
  struct sockaddr_in addr = {
      ...
      .sin_addr = {.s_addr = INADDR_ANY}}; // no need for the t->ip psk it's "server", so he binds to all available interfaces..
...
{
  LOG_I(HW, "Start CLIENT \n");

  rfsimulator_state_t *t = device->priv; // receive the rfsimulator.
  ...
  addr.sin_addr.s_addr = inet_addr(
      t->ip); // tries to connect just to the server's IP directly ( already known ).. 127.0.0.1..no discovering service needed..
...
static int rfsimulator_write_internal(rfsimulator_state_t *t,
                                      ...
                                      int flags,
                                      bool alreadyLocked)
{ // send rf signal ( samplesVoid ) through connection sockets, at the time : timestamp,
  // nsamps nbr of samples to send from each antenna / channel | number of samples that represents the rf signal

  LOG_W(HW, "rfsimulator_write_internal start \n");
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  LOG_I(HW, "Writing TS ( passed in in rfsimulator_write args ) timestamp : %ld  \n", timestamp);

  sleep(1);

  if (!alreadyLocked) // locking the connection socket.
    pthread_mutex_lock(&Sockmutex);

  LOG_I(HW, "sending %d samples at time: %ld, nbAnt %d\n", nsamps, timestamp, nbAnt);

  for (int i = 0; i < FD_SETSIZE; i++) { // iterating over the device connection sockets
    buffer_t *b = &t->buf[i];

    if (b->conn_sock >= 0) {
      samplesBlockHeader_t header = {t->typeStamp, nsamps, nbAnt, timestamp};
      LOG_I(HW,
            "Header we are sending in write internal: typeStamp : %ld, nsamps : %ld, nbAnt : %d, timestamp: %ld   \n",
            t->typeStamp,
            nsamps,
            nbAnt,
            timestamp);

      // sending the header through this connection socket..
      fullwrite(b->conn_sock, &header, sizeof(header), t);

      sample_t tmpSamples[nsamps][nbAnt]; // matrix of all the rf signals to send over the nbAnt antennas ( each rf signal contains
                                          // nsamps IQ sample )

      // iterate over the rfsignal to sent ( stored in samplesVoid = nbAnt) to retreive the samples
      for (int a = 0; a < nbAnt; a++) {
        sample_t *in = (sample_t *)samplesVoid[a]; // sample associated with the anetnna a in the samplesVoid
        // in = rf signal
      ...
      if (b->conn_sock >= 0) {
        // sending the rf signal through this connection socket..
  ...
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  sleep(1);
  LOG_W(HW, "rfsimulator_write_internal ends \n");
...
static int rfsimulator_write(openair0_device *device,
                             ...
                             int flags)
{
  LOG_W(HW, "rfsimulator_write start\n");
  rfsimulator_state_t *t = device->priv;
  // LOG_I(HW, "Reading time stamp lastWroteTS : %ld  \n", t->lastWroteTS);
  // sleep(10000);
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  LOG_I(HW, "Writing TS ( passed in in rfsimulator_write args ) timestamp : %ld  \n", timestamp);
...
{ // read rf signal from connected sockets ( ones with incoming events )..

  // nsamps nbr of samples to receive from each channel..
  LOG_W(HW, "flushInput start \n");
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  sleep(1);
  // Process all incoming events on sockets ( there is one incoming event from each socket..)
  // store the data in lists
  struct epoll_event events[FD_SETSIZE] = {
      {0}}; // there is maximum fd_setsize incoming event ( nbr of possible connection sockets )..

  int nfds = epoll_wait(t->epollfd, events, FD_SETSIZE, timeout);
  // nfds number of sockets that has incoming events ( they are ready to read from them their rf signal..)
  // printf("nfds nbr of sockets with incoming events in original project: %d \n", nfds);
  if (nfds == -1) {
  ...
  for (int nbEv = 0; nbEv < nfds; ++nbEv) {
    // iterate over the sockets with incoming events
    int fd = events[nbEv].data.fd;

    if (events[nbEv].events & EPOLLIN
        && fd == t->listen_sock) { // if this is the server that received connection requests..=> create new connection sockets and
                                   // send to the client current time..client ydeblocka w ydir read, yl9a current time apres ykemel
                                   // khdmto ...
      int conn_sock;
      ...
      c16_t v = {0}; // v is a sample...complex number v = I + Qi
      void *samplesVoid[t->tx_num_channels]; // tableau of nbAnt case.

      for (int i = 0; i < t->tx_num_channels; i++)
        samplesVoid[i] = (void *)&v;
      // send it with an empty rf signal ( samplesVoid  ) ? sinon we can't send just the current time tout seul..we need an rf
      // signal..

      LOG_I(HW, "first writing ( current time sent ) timestamp lastWroteTS : %ld  \n", t->lastWroteTS);
      sleep(1);

      rfsimulator_write_internal(t, t->lastWroteTS > 1 ? t->lastWroteTS - 1 : 0, samplesVoid, 1, t->tx_num_channels, 1, false);
    } else {
      // this is a connection socket ( on client or server ) that received an rf signal.

      if (events[nbEv].events & (EPOLLHUP | EPOLLERR | EPOLLRDHUP)) { // problem flags...
        socketError(t, fd);
        continue;
      }

      buffer_t *b = &t->buf[fd]; // this is a connection socket.
      LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);

      LOG_I(HW, "socket timeStamp  ( last received ) lastReceivedTS : %ld  \n", b->lastReceivedTS);
      sleep(1);

      if (b->circularBuf == NULL) { // to check if it is a connected socket.
      ...
      // read from this connected socket the rf signal..MSG_DONTWAIT to make it non blocking ( we return instead of waiting for data
      // to arrive), cuz we can read from it later ( in the loop of rfsim_read ) if the rf signal that we ve read here is not
      // complete..it's to continue executing other tasks instead of blocking it here in the receive...

      ssize_t sz = recv(fd, b->transferPtr, blockSz, MSG_DONTWAIT);
      // nsz : umber of bytes we ve read
      if (sz < 0) {
        if (errno != EAGAIN) {
      ...
      b->transferPtr += sz; // number of bytes we ve read
      ...
      if (b->headerMode == true && b->remainToTransfer == 0) {
        ...
        if (t->nextRxTstamp == 0) { // First block in UE, resync with the eNB current TS
          // here when we receive the current time in the client/UE..
          t->nextRxTstamp = b->th.timestamp > nsamps_for_initial ? b->th.timestamp - nsamps_for_initial : 0;
          // initially stays 0, cuz b->th.timestamp = 0, nd nsamps_for_intiial = 30720 ( passed in the flushInput args, which were
          // passed before "nsamps" in the call from rfsimulator_read)
          b->lastReceivedTS = b->th.timestamp > nsamps_for_initial ? b->th.timestamp : nsamps_for_initial;
          // here lastReceivedTS got nsamps_for_intial = 30720..

          LOG_W(HW, "UE got first timestamp: starting at %lu\n", t->nextRxTstamp);
          b->trashingPacket = true;

        } else if (b->lastReceivedTS < b->th.timestamp) {
          // server shouldn't enter here in his second read, cuz his LastReceivedTS = 0, nd the received timestamp fromm the client
          // ( in the header ) waas 0 (in his first write.....)
          ...
          // we received data before its header ( before we received its timestamp! )
          LOG_W(HW, "Received Rx/Tx synchro out of order\n");
          b->trashingPacket = true;
        } else if (b->lastReceivedTS == b->th.timestamp) {
          // server enters here in his second read..
          //  normal case
        } else {
      ...
      if (b->headerMode == false) {
        // actual reception of the data...we update just the lastReceivedTS

        if (!b->trashingPacket) {
          b->lastReceivedTS = b->th.timestamp + b->th.size
                              - byteToSample(b->remainToTransfer, b->th.nbAnt); // b->th.size (= nsamps la taille du rf signal)
          LOG_I(HW, "UEsock: %d Set b->lastReceivedTS %ld\n", fd, b->lastReceivedTS);
        }

        if (b->remainToTransfer == 0) {
          LOG_I(HW, "UEsock: %d Completed block reception: %ld\n", fd, b->lastReceivedTS);
          b->headerMode = true;
  ...
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  sleep(1);
  LOG_W(HW, "flushInput ends \n");

  return nfds > 0;
}

static int rfsimulator_read(openair0_device *device, openair0_timestamp *ptimestamp, void **samplesVoid, int nsamps, int nbAnt)
{ // Read the received rf signal from all connection sockets ( on client/server ) + handle the special case : listening socket on
  // the serverr..
  //  nsamps : nbr of samples to receive from each channel.....is it FIXED in the upper layers ?
  // ptimestamp : current timestamp of the current event "read".
  // nextrxtstamp: timestamp of the next event "read" that rfsimulator need to perfom next time.
  LOG_W(HW, "rfsimulator_read start\n");
  ...
  rfsimulator_state_t *t = device->priv;
  LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
  LOG_I(HW, "current (previous of nextRxTs) Reading TS (passed as arg in rfsimulator_read ) ptimestamp  : %ld \n", *ptimestamp);
  LOG_I(HW, " trying to read nsamps samples : %d (passed as arg in rfsimulator_read ) \n", nsamps);
  ...
  int first_sock; // to handle first time our device ( server/UE ) is turned on...mainly to handle the server even if the client is
                  // turned on before the server.

  for (first_sock = 0; first_sock < FD_SETSIZE; first_sock++)
    if (t->buf[first_sock].circularBuf != NULL)
      break; // we break when there is already a connection socket, either at the server side or client side..stored in first_sock
             // now.

  if (first_sock == FD_SETSIZE) { // we saw all the buffer ( we reached the end ) nd there is no connection sockets to read rf
                                  // signal from ( on the
    // server, cuz client should be blocked in the startClient..)
    // no connected device (we are eNB, no UE is connected)....we generate our own temporary rf signal in the samplesVoid..
    // LOG_W(HW, "Tjrs dkhel malgre client cha3el...\n");

    if (t->nextRxTstamp == 0)
      LOG_W(HW, "No connected device, generating void samples...\n");

    if (!flushInput(t, t->wait_timeout, nsamps)) {
      // flushInput is false meaning, there is not yet incoming client requests for the listening socket.
      // otherwise true, meaning we handled in th eflushInput the client requests...
      // LOG_W(HW, "dkhel malgre client cha3el 9bl srvr == flushInput rej3et false tjrs m3a lwl...\n");
      ...
      if (((t->nextRxTstamp / nsamps) % 100) == 0)
        LOG_I(HW, "No UE, Generated void samples for Rx: %ld\n", t->nextRxTstamp);

      *ptimestamp = t->nextRxTstamp - nsamps;
      LOG_I(HW, "After the first read");
      LOG_I(HW, "next Reading TS nextRxTstamp : %ld , last Writing TS lastWroteTS : %ld  \n", t->nextRxTstamp, t->lastWroteTS);
      LOG_I(HW, "current Reading TS ptimestamp : %ld \n", *ptimestamp);

      sleep(1);
      LOG_W(HW, "first rfsimulator_read ends \n");
    ...
    // here e treat the connection socket psk we find it (indice in t->buf < FD_SETSIZE )

    // there is at least one connection socket, stored in first_sock ( on clients/server ).. but we re-iterate over all the sockets
    // 7na..
    bool have_to_wait;

    do { // this loop is to ensure that all the connection sockets have to receive completly their rf signals to complete the
         // reading process.
      have_to_wait = false;

      for (int sock = 0; sock < FD_SETSIZE; sock++) {
        // iterate through all the conenction sockets..and read the received signal from them ( put it in b->CircularBuf)
        buffer_t *b = &t->buf[sock];

        if (b->circularBuf) // if there is an initialized buffer (so it s a connected socket )..
          if (t->nextRxTstamp + nsamps > b->lastReceivedTS) {
            // meaning we didn't red yet from the socket b.

            // to detect if we it reads all its received signal..else we should
            // wait at it and receive its signal in the b->circularBuf ..

            // if the next expected timestamp to receive data + number of samples to receive > time stamp of last received data,
            // meaning that we still need to receive more data on this particular socket..
            have_to_wait = true; // have to wait to read from the first connection socket that received data.
            break;
          }
      }
      ...
      if (have_to_wait)
        /*printf("Waiting on socket, current last ts: %ld, expected at least : %ld\n",
          ...
          t->nextRxTstamp+nsamps);
        */
        flushInput(t, 3, nsamps); // Read rf signal from this socket..
    } while (have_to_wait);
  }
  ...
  // Summarizing read : copying all the received data ( rf signal ) to the samplesVoid buffer...

  // Add all input nodes signal in the output buffer
  for (int sock = 0; sock < FD_SETSIZE; sock++) {
    buffer_t *ptr = &t->buf[sock];

    if (ptr->circularBuf) { // check wether it is an initilizaed connection socket.
      // what does this mean, how we are ensuring that the conenction socket is receiving an rf signal.
  ...
  *ptimestamp = t->nextRxTstamp; // return the time of the first sample
  ...
  LOG_W(HW, "rfsimulator_read ends \n");
...
{ // this will init the simulator in the device
  ...
  rfsimulator_readconfig(rfsimulator); // initial config = channel modeling + determine wether it's UE or gNb
  ...
  // initializing the device's rfsimulator ( tranceiver ) needed functions
  device->trx_start_func = rfsimulator->typeStamp == ENB_MAGICDL ? startServer : startClient; // starting the tranceiver, rfsim
  device->trx_get_stats_func = rfsimulator_get_stats;
  device->trx_reset_stats_func = rfsimulator_reset_stats;
  device->trx_end_func = rfsimulator_end; // end the tranceiver.
  device->trx_stop_func = rfsimulator_stop;
  device->trx_set_freq_func = rfsimulator_set_freq;
  device->trx_set_gains_func = rfsimulator_set_gains;
  device->trx_write_func = rfsimulator_write; // write rfsignals through the traceiver.
  device->trx_read_func = rfsimulator_read; // read rfsignals through the traceiver.
  /* let's pretend to be a b2x0 */
  device->type = RFSIMULATOR; // type of the device, UE or gNb
  openair0_cfg[0].rx_gain[0] = 0;
  device->openair0_cfg = &openair0_cfg[0];
  device->priv = rfsimulator;
  device->trx_write_init = rfsimulator_write_init; // init the writing

  for (int i = 0; i < FD_SETSIZE; i++) // creating fd_setsize connection sockets for the simulator.
    rfsimulator->buf[i].conn_sock = -1;

  AssertFatal((rfsimulator->epollfd = epoll_create1(0)) != -1,
              ""); // creates a new epoll instance and returns its file descriptor epollfd
